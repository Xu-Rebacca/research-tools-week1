...
实验仓库创建于本地，采用 \texttt{git init -b main} 初始化，远程仓库以本地裸仓库（\texttt{git init --bare}）模拟 GitHub 远程，操作流程与真实 GitHub 一致；Codex 任务由于网络环境限制（详见第 4 节），在配置国内供应商后即可完成。
...
\subsubsection{登录和检查状态}

在项目目录中使用 \texttt{codex login status} 检查登录状态。登录方式可选择 ChatGPT 登录或 API Key 登录；若使用 API Key，应把密钥放入环境变量并通过标准输入登录，\textbf{不要把密钥写进实验报告、Git 仓库、截图或配置文件}。本实验中登录凭据已配置（此处省略密钥内容，防止泄露）。

\begin{lstlisting}[caption={检查 Codex 登录状态}]
$ codex login status
Logged in using an API key - sk-proj-***（密钥已脱敏）
\end{lstlisting}

\subsubsection{尝试执行 Codex 任务}
...
\textbf{说明：}由于 API Key 由用户自行持有（且实验要求不得泄露），供应商切换为 GUI 手动操作步骤；本报告提交时，Codex 在线编程任务因网络与供应商配置尚未完成，待配置国内供应商后即可按 4.3 节步骤完成词频统计程序任务并补充运行结果。
...
\begin{enumerate}
    ...
    \item \textbf{AI 辅助编程工具}：完成了 Codex CLI 的安装与配置，理解其“权限确认—命令审阅—运行测试—Git 复核”的可靠使用流程；安装了 CC Switch 并了解了供应商配置与切换方法。由于当前网络无法连接官方端点，Codex 在线任务待配置国内供应商后完成，这也让我认识到科研工具使用中网络环境与镜像/中转配置的重要性。
    \item \textbf{遇到的问题与解决}：① 官方端点连接失败——通过切换国内供应商解决；② Windows 下 Git 的 LF/CRLF 换行符警告——属正常现象，不影响功能；③ 脚本写入 README 导致中文乱码——统一使用 UTF-8 编码并修复提交；④ PowerShell 控制台中文路径乱码——设置 \texttt{chcp 65001} 与 UTF-8 输出编码解决。
\end{enumerate}
